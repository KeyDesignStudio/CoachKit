export function getPageTitleFromPath(pathname: string): string {
  ...
  if (pathname.startsWith('/coach/assistant')) return 'Assistant';
  if (pathname.startsWith('/coach/challenges')) return 'Challenges';
  ...
  if (pathname.startsWith('/athlete/training-request')) return 'Training Request';
  if (pathname.startsWith('/athlete/intake')) return 'Training Request';
