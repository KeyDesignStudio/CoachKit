...
export function AthleteMonthDayCell({
  ...
  // On mobile month (7-column grid), each cell is narrow (~55px at 390px).
  // Keep the scan-only icon row compact to avoid overflowing into adjacent cells.
  const mobileIconItems = sortedItems.slice(0, 2);
  const mobileExtraCount = Math.max(0, sortedItems.length - mobileIconItems.length);
  ...
  return (
    ...
    >
      ...
      <div data-mobile-month-scan-only="true" className="md:hidden">
      ...
      {/* Desktop month (detailed): keep existing rows */}
