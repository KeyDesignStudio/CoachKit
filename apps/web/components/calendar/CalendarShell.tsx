...
export function CalendarShell({ variant, children, className, structureClassName, ...props }: CalendarShellProps) {
  const structureClasses =
    variant === 'week'
      ? 'rounded bg-[var(--bg-structure)] p-3'
      : 'overflow-hidden rounded bg-[var(--bg-structure)] p-px';

  return (
    <div
      {...props}
      data-cal-shell="true"
      className={cn('rounded border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-3', className)}
    >
