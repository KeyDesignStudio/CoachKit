/**
 * DEV/TEST ONLY – resets Athlete 1 for Strava draft-workflow testing
 *
 * Goal:
 * - Re-seed Athlete 1 planned sessions for a fixed date range
 * - Detach Strava CompletedActivity rows in that window so poll can re-match
 * - Ensure matched items become COMPLETED_SYNCED_DRAFT (not coach-visible) until athlete confirms
 *
 * Target:
 ...
 * - Does NOT mutate CompletedActivity.notes / painFlag / metricsJson
 *
 * Run (from repo root):
...
import { CalendarItemStatus, CompletionSource, PlanWeekStatus, Prisma, PrismaClient } from '@prisma/client';
...
const rangeEndExclusive = new Date('2026-01-09T00:00:00.000Z'); // exclusive (end of 2026-01-08)
...
async function main() {
  if (process.env.CONFIRM_RESET !== 'YES') {
    throw new Error(
      "Refusing to run without explicit confirmation. Re-run with CONFIRM_RESET=YES (dev/test only)."
    );
  }

  console.log('[reset-athlete1-strava-draft-test-range] Starting...');
  ...
  console.log(`[reset-athlete1-strava-draft-test-range] Deleted CalendarItems: ${deleted.count}`);

  // 2) Detach STRAVA CompletedActivities in that time window
  const detached = await prisma.completedActivity.updateMany({
    where: {
      athleteId: ATHLETE_ID,
  ...
  console.log(`[reset-athlete1-strava-draft-test-range] Detached STRAVA CompletedActivities: ${detached.count}`);

  // 3) Insert one planned CalendarItem per day in the range
  ...
  const created = await prisma.calendarItem.createMany({ data: items });
  console.log(`[reset-athlete1-strava-draft-test-range] Created CalendarItems: ${created.count}`);

  // 4) Publish PlanWeeks covering the range
  ...
  for (const weekStart of weekStarts) {
    ...
    console.log(`[reset-athlete1-strava-draft-test-range] Published PlanWeek: ${ws.toISOString().slice(0, 10)}`);
  }

  // 5) Clear StravaConnection.lastSyncAt to force re-poll
  const updatedConn = await prisma.stravaConnection.updateMany({
    where: { athleteId: ATHLETE_ID },
    data: { lastSyncAt: null },
  });

  console.log(`[reset-athlete1-strava-draft-test-range] Cleared StravaConnection.lastSyncAt: ${updatedConn.count} row(s)`);
  console.log('[reset-athlete1-strava-draft-test-range] Done.');
}

main()
  .catch((err) => {
    console.error('[reset-athlete1-strava-draft-test-range] Failed:', err);
    process.exitCode = 1;
  })
  .finally(async () => {
