...
describe('AI Plan Builder v1 (admin reset athlete endpoint)', () => {
  ...
  beforeAll(async () => {
    ...
    // Seed APB state to prove reset works.
    ...
    // Ensure proposal/audit records exist.
    ...
    // Seed invocation + rate limit events (metadata-only) tied to this athlete.
  ...
  afterAll(async () => {
    ...
    // Clean up APB tables first (FK deps), then fixture rows.
